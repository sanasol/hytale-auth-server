...
public class DualAuthPatcher {
        ...
        private static byte[] generateDualAuthContext() {
                ...
                // NEW: Server Validation Mode flag for startup bypass
                cw.visitField(Opcodes.ACC_PRIVATE | Opcodes.ACC_STATIC | Opcodes.ACC_FINAL,
                                "serverValidationMode", "Ljava/lang/ThreadLocal;", null, null).visitEnd();
                ...
                mv.visitInsn(Opcodes.DUP);
                mv.visitMethodInsn(Opcodes.INVOKESPECIAL, "java/lang/ThreadLocal", "<init>", "()V", false);
                mv.visitFieldInsn(Opcodes.PUTSTATIC, CONTEXT_CLASS, "serverValidationMode", "Ljava/lang/ThreadLocal;");
                ...
                // public static void setServerValidationMode(boolean val)
                mv = cw.visitMethod(Opcodes.ACC_PUBLIC | Opcodes.ACC_STATIC,
                                "setServerValidationMode", "(Z)V", null, null);
                mv.visitCode();
                mv.visitFieldInsn(Opcodes.GETSTATIC, CONTEXT_CLASS, "serverValidationMode", "Ljava/lang/ThreadLocal;");
                mv.visitVarInsn(Opcodes.ILOAD, 0);
                mv.visitMethodInsn(Opcodes.INVOKESTATIC, "java/lang/Boolean", "valueOf", "(Z)Ljava/lang/Boolean;",
                                false);
                mv.visitMethodInsn(Opcodes.INVOKEVIRTUAL, "java/lang/ThreadLocal", "set", "(Ljava/lang/Object;)V",
                ...
                // public static boolean isServerValidationMode()
                mv = cw.visitMethod(Opcodes.ACC_PUBLIC | Opcodes.ACC_STATIC,
                                "isServerValidationMode", "()Z", null, null);
                mv.visitCode();
                mv.visitFieldInsn(Opcodes.GETSTATIC, CONTEXT_CLASS, "serverValidationMode", "Ljava/lang/ThreadLocal;");
                mv.visitMethodInsn(Opcodes.INVOKEVIRTUAL, "java/lang/ThreadLocal", "get", "()Ljava/lang/Object;",
                                false);
                mv.visitInsn(Opcodes.DUP);
                Label isServerNotNull = new Label();
                mv.visitJumpInsn(Opcodes.IFNONNULL, isServerNotNull);
                mv.visitInsn(Opcodes.POP);
                mv.visitInsn(Opcodes.ICONST_0);
                mv.visitInsn(Opcodes.IRETURN);
                mv.visitLabel(isServerNotNull);
                mv.visitTypeInsn(Opcodes.CHECKCAST, "java/lang/Boolean");
                mv.visitMethodInsn(Opcodes.INVOKEVIRTUAL, "java/lang/Boolean", "booleanValue", "()Z", false);
                mv.visitInsn(Opcodes.IRETURN);
                mv.visitMaxs(2, 0);
        ...
        private static byte[] generateDualAuthHelper() {
                ...
                // if (DualAuthContext.isServerValidationMode()) return true;
                mv.visitMethodInsn(Opcodes.INVOKESTATIC, CONTEXT_CLASS, "isServerValidationMode", "()Z", false);
                Label skipServerCheck = new Label();
                mv.visitJumpInsn(Opcodes.IFEQ, skipServerCheck);
                mv.visitInsn(Opcodes.ICONST_1);
                mv.visitInsn(Opcodes.IRETURN);
                mv.visitLabel(skipServerCheck);
                        ...
                        if (mn.name.startsWith("validate") &&
                                ...
                                if (returnType.getSort() == Type.OBJECT) {
                                        ...
                                        hook.add(new MethodInsnNode(Opcodes.INVOKESTATIC,
                                                        ...
                                                        "(Ljava/lang/String;)Z",
                                                        false));
                                        LabelNode checkServerMode = new LabelNode(); // Jump here if TRUST_ALL is false
                                        hook.add(new JumpInsnNode(Opcodes.IFEQ, checkServerMode));

                                        // Trust All is TRUE -> Allow
                                        hook.add(new JumpInsnNode(Opcodes.GOTO, allowOmni));

                                        // Trust All is FALSE -> Check Server Mode
                                        hook.add(checkServerMode);
                                        hook.add(new MethodInsnNode(Opcodes.INVOKESTATIC, CONTEXT_CLASS,
                                                        "isServerValidationMode", "()Z", false));
                                        // If Server Mode is TRUE -> Allow, Else -> Strict Reject
                                        hook.add(new JumpInsnNode(Opcodes.IFNE, allowOmni));
                                        hook.add(new JumpInsnNode(Opcodes.GOTO, strictOmni));
        ...
        private static byte[] patchServerAuthManager(byte[] classBytes) {
                try {
                        ...
                        for (MethodNode method : classNode.methods) {
                                ...
                                // Patch initialize() to set ServerContext flags
                                if (method.name.equals("initialize") && method.desc.equals("()V")) {
                                        if (injectServerContextFlags(method)) {
                                                modified = true;
                                                patchedMethods.add("ServerAuthManager.initialize");
                                                System.out.println(
                                                                "  [ServerAuthManager] Patched initialize() with DualAuthContext flags");
                                        }
                                }
        ...
        /**
         * Inject server context flags into a method.
         * sets serverValidationMode to true at start, and false before returns.
         */
        private static boolean injectServerContextFlags(MethodNode method) {
                // 1. Inject setServerValidationMode(true) at start
                InsnList startInject = new InsnList();
                startInject.add(new InsnNode(Opcodes.ICONST_1));
                startInject.add(new MethodInsnNode(Opcodes.INVOKESTATIC, CONTEXT_CLASS,
                                "setServerValidationMode", "(Z)V", false));

                AbstractInsnNode first = method.instructions.getFirst();
                method.instructions.insertBefore(first, startInject);

                // 2. Inject setServerValidationMode(false) before returns
                boolean patchedReturns = false;
                for (AbstractInsnNode insn : method.instructions.toArray()) {
                        if (insn.getOpcode() == Opcodes.RETURN) {
                                InsnList endInject = new InsnList();
                                endInject.add(new InsnNode(Opcodes.ICONST_0));
                                endInject.add(new MethodInsnNode(Opcodes.INVOKESTATIC, CONTEXT_CLASS,
                                                "setServerValidationMode", "(Z)V", false));
                                method.instructions.insertBefore(insn, endInject);
                                patchedReturns = true;
                        }
                }
                return patchedReturns;
        }
}
